...
fun FavoritesScreen(
    ...
    onFavoriteClick: (FavoriteCrossingEntity) -> Unit
) {
    val favorites by viewModel.favorites.collectAsState()

    Scaffold(
        topBar = { GranicaHeader(title = stringResource(R.string.favorites_title)) }
    ) { padding ->
                ...
                items(favorites) { favorite ->
                    Card(
                        ...
                        Row(
                            ...
                            Text(countryFlagEmoji(favorite.countryCode), style = MaterialTheme.typography.headlineSmall)
                            Text(favorite.name, style = MaterialTheme.typography.titleMedium, modifier = Modifier.padding(start = 12.dp))
                        }
                    }
                }
            }
